...
const nodeMeta = {
    ...
    N07: { module: 1, section: '§1-6', title: '水波的干涉與繞射', lab: ['ripple', 'water_interference'] },
    ...
    N21: { module: 2, section: '延伸', title: '都卜勒效應', lab: null }
};
...
const labMeta = {
    ...
    water_interference: { name: '水波干涉探究模擬器', desc: '成德高中 陳家騏老師提供。凍結波形、點擊畫布任意位置即時測量波程差與相位差，結合理論雙曲線腹節線驗證與 1D/2D 視角切換。' },
    sound: { name: '聲波的傳播', desc: '疏密相間的球面波、聲速與溫度、進入密介質變快變長（與光相反），以及回聲與聲納的來回時間。' },
    longitudinal: { name: '縱波：空氣分子、位移與壓力', desc: '四層對照圖：未振動前分子位置、振動後實際疏密與位移箭頭、位移圖、壓力圖。' },
...
const modulesData = {
    1: {
        ...
        examQuestions: [
            ...
            { node: ['N06'], year: '110 指考（答對率 82%）', title: '水波槽深水進淺水之折射與波長變化', image: 'assets/images/exams/drte-110-17.png', imageCaption: '【110 指考 第 17 題】大考中心原卷試題圖（水波槽深淺水界面 ab 折射選項圖）', desc: '<strong>解題特徵引導：</strong>水波由深水區進入淺水區，波速變慢、頻率由波源決定恆定，由 $v=f\\lambda$ 得波長變短；由折射定律 $\\sin\\theta_{\\text{深}}/\\sin\\theta_{\\text{淺}} = v_{\\text{深}}/v_{\\text{淺}} > 1$ 知折射線偏向法線。' },
            { node: ['N07'], year: '112 分科（答對率 68%）', title: '機車干涉型消音器之破壞性干涉路徑差', image: 'assets/images/exams/ast-112-11.png', imageCaption: '【112 分科 第 11 題】大考中心原卷試題圖（含圖 4 機車消音器）', desc: '<strong>解題特徵引導：</strong>聲波分兩路傳播後在出口會合，破壞性干涉相消條件為波程差 $\\Delta L = (n+\\frac{1}{2})\\lambda$。當主要噪音波長 $\\lambda = 1.0$ m 時，最小路徑差為 $0.50$ m。' },
    ...
    2: {
        ...
        formulas: [
            ...
            },
            {
                node: 'N21',
                name: '都卜勒效應：頻率公式',
                formula: "f' = \\dfrac{v \\pm v_o}{v \\mp v_s}\\, f",
                anchor: '分子管觀察者、分母管波源；先問「這個運動會讓 f′ 變大還是變小」再決定正負號。',
                probe: {
                    q: '波源朝著靜止的觀察者前進時，空氣中的「聲速」會如何改變？',
                    opts: [
                        { t: '(A) 變快，因為波源推著波前進' },
                        { t: '(B) 不變，波速只由介質決定', c: 1 },
                        { t: '(C) 變慢，因為前方的波前被擠密' }
                ...
                desc: '<b>符號規則不必死背</b>：把 $v$（介質中的波速）寫下來後，只問一句「這個運動會讓 $f^{\\prime}$ 變大還是變小」。<br>• 觀察者<b>朝波源</b>移動 $\\Rightarrow$ 分子取 $v + v_o$；遠離則取 $v - v_o$。<br>• 波源<b>朝觀察者</b>移動 $\\Rightarrow$ 分母取 $v - v_s$；遠離則取 $v + v_s$。<br><b>只有徑向速度算數</b>：垂直於連心線的速度分量不造成頻率變化。所以救護車與你距離最近的那一瞬間，聽到的恰好是原本的 $f$，音調由高轉低的轉折點就在那裡。'
            },
            {
                node: 'N21',
                name: '波源移動與觀察者移動：不是同一回事',
                formula: '\\text{波源移動} \\Rightarrow \\lambda\\ \\text{改變} \\qquad \\text{觀察者移動} \\Rightarrow \\lambda\\ \\text{不變}',
                anchor: '兩者都會讓接收頻率改變，但只有波源移動會真的改變空間中的波長。',
                desc: '<b>波源移動</b>：波源每送出一個波前就往前挪一點，前方波前被擠密（$\\lambda$ 變短）、後方被拉疏（$\\lambda$ 變長）。空間中的波形<b>真的</b>不再是同心圓。<br><b>觀察者移動</b>：空間中的波前仍是同心圓、間隔完全沒變；只是觀察者迎上去時單位時間<b>掃過</b>更多波前，因此測到較高的頻率。<br><b>為什麼要分清楚</b>：題目問「波長變不變」時，答案完全取決於是誰在動——這是本節最常見的陷阱。'
            }
        ],
        conceptMap: `
            <div class="concept-map-container">
        ...
        keyFeatures: [
            ...
            {
                node: 'N21',
                feature: '題目描述火車鳴笛通過、救護車經過身邊，或問「聽到的頻率」與「波長」怎麼變。',
                bridge: '先判斷是<b>誰在動</b>：波源動才改變波長，觀察者動只改變單位時間掃過的波前數。再用 $f^{\\prime} = \\dfrac{v \\pm v_o}{v \\mp v_s} f$，正負號一律由「接近變高、遠離變低」反推。',
                trap: '⚡ 避坑指南：都卜勒效應<b>完全不改變波速</b>。看到「聲速變快」的選項直接刪；也別忘了只有徑向速度有效，正側方經過的瞬間頻率恰為原值。'
            }
        ],
        extensions: `
            <div class="edu-table-container">
        ...
        examQuestions: [
            ...
            { node: ['N10'], year: '109 指考（答對率 56%）', title: '長笛基音與單簧管第一泛音同頻', image: 'assets/images/exams/drte-109-06.png', imageCaption: '【109 指考 第 15 題】大考中心原卷試題圖（長笛開管基音 vs 單簧管閉管第一泛音）', desc: '<strong>解題特徵引導：</strong>長笛為開管取 $n=1$、單簧管為閉管的第一泛音取 $m=3$，令 $\\dfrac{v}{2L} = \\dfrac{3v}{4L\'}$ 得 $L : L\' = 2 : 3$。答對率不到六成，失分主因是把閉管第一泛音誤取成 $m=2$。' },
            { node: ['N08', 'N20'], year: '111 分科（探究實作題）', title: '瑞立矩形開口擴音器聲波繞射角分析', image: 'assets/images/exams/ast-111-25.png', imageCaption: '【111 分科 第 25 題】大考中心原卷試題圖（含題組圖 13 瑞立擴音器與理論模型）', desc: '<strong>解題特徵引導：</strong>將人嘴喊話視為寬 $a=6$ cm 矩形開口，頻率 $3$ kHz、聲速 $340$ m/s 算出波長 $\\lambda \\approx 11.3$ cm。因 $\\lambda/a \\approx 1.89 > 1$，單狹縫繞射極小值條件無解，開口等效為點波源，水平方向展開張角可達 $180^\\circ$。' },
    ...
    3: {
        ...
        formulas: [
            ...
            },
            {
                node: 'N14',
                name: '視深公式',
                formula: '\\dfrac{h^{\\prime}}{h} = \\dfrac{n^{\\prime}}{n} \\quad(\\text{觀察者幾乎在正上方})',
                anchor: '在空氣中看水裡的魚會覺得變淺；在水中看岸上的鳥會覺得變高。',
                desc: '$h$ 為實深、$h^{\\prime}$ 為視深，$n$ 為物體所在介質、$n^{\\prime}$ 為觀察者所在介質的折射率。<br>• 空氣看水中物（$n=4/3 \\to n^{\\prime}=1$）：$h^{\\prime} = \\frac{3}{4}h$，<b>變淺</b>。<br>• 水中看空氣中物（$n=1 \\to n^{\\prime}=4/3$）：$h^{\\prime} = \\frac{4}{3}h$，<b>變高</b>。<br><b>連帶效應</b>：垂直方向的長度與速度都會依同一比例縮放，水平方向則不變 $\\Rightarrow$ 插在水杯中的鉛筆看起來被折斷。'
            },
            {
                node: 'N14',
                name: '光的色散',
        ...
        `,
        examQuestions: [
            { node: ['N13'], year: '109 指考（答對率 62%）', title: '兩塊三角玻璃夾一層介質的折射光線', image: 'assets/images/exams/drte-109-14.png', imageCaption: '【109 指考 第 14 題】大考中心原卷試題圖（含圖 6 稜鏡夾層折射線條選填）', desc: '<strong>解題特徵引導：</strong>因兩斜面平行，入射線與出射線必平行，答案只可能落在同一組平行線中。再分三種情況：$n_1 = n_2$ 不偏折、$n_1 < n_2$ 與 $n_1 > n_2$ 各偏一邊，故答案為中間那一組（4、5、6）。' },
            { node: ['N13', 'N16'], year: '112 分科（多選）', title: '圓柱塑膠瓶裝水聚光與司乃耳定律', image: 'assets/images/exams/ast-112-18.png', imageCaption: '【112 分科 第 18 題】大考中心原卷試題圖（含圖 5 圓柱塑膠瓶光路與表 2）', desc: '<strong>解題特徵引導：</strong>平行陽光穿入裝水圓柱瓶折射會聚於焦點。光跨越介質時頻率與週期不變（$T_a = T_w$），水折射率較大故光速減慢（$v_w < v_a$），折射角滿足 $n_a\\sin\\theta_a = n_w\\sin\\theta_w$ 且偏向法線（$\\theta_w < \\theta_a$）。' },
            { node: ['N14'], year: '111 分科（生活情境題）', title: '防疫透明壓克力隔板視位移量值與折射率', image: 'assets/images/exams/ast-111-16.png', imageCaption: '【111 分科 第 16 題】大考中心原卷試題圖（含圖 8 壓克力隔板實景照片）', desc: '<strong>解題特徵引導：</strong>垂直正視厚度 $h$、折射率 $n$ 的透明隔板，板後格線視位移量 $\\Delta y = h(1 - 1/n)$，大小只取決於板厚與折射率，與物距無關；量測視位移與厚度的比值即可精準反求折射率 $n = \\frac{1}{1 - \\Delta y/h}$。' },
    ...
    4: {
        ...
        `,
        examQuestions: [
            { node: ['N18'], year: '113 分科（單選）', title: '光波跨介質波長變化、薄膜干涉與色散', image: 'assets/images/exams/ast-113-02.png', imageCaption: '【113 分科 第 2 題】大考中心原卷試題圖（光跨介質、薄膜干涉與波粒二象性）', desc: '<strong>解題特徵引導：</strong>單色光由空氣進入玻璃，頻率不變、光速變慢，波長隨之縮短（$\\lambda\' = \\lambda/n$）；肥皂泡表面七彩為薄膜干涉、彩虹為折射與反射色散、雙狹縫干涉證實波動性而光電效應證實粒子性。' },
            { node: ['N19'], year: '113 分科（多選）', title: '雙狹縫干涉條紋間距與光程差分析', image: 'assets/images/exams/ast-113-14.png', imageCaption: '【113 分科 第 14 題】大考中心原卷試題圖（雙狹縫干涉條紋間距與光程差）', desc: '<strong>解題特徵引導：</strong>由 $\\Delta y = \\frac{\\lambda L}{d}$ 判斷：狹縫間隔 $d$ 增加條紋變密；浸入水中波長變短條紋變密；紅光改綠光（波長變短）條紋變密；白光照射時藍光亮紋比紅光更接近中央；相鄰兩暗紋光程差相差一個完整波長 $\\lambda$。' },
            { node: ['N19', 'N20'], year: '110 指考（多選）', title: '雷射雙狹縫干涉與單狹縫繞射圖樣綜合運算', image: 'assets/images/exams/drte-110-23.png', imageCaption: '【110 指考 第 23 題】大考中心原卷試題圖（雙狹縫干涉與單狹縫繞射綜合運算）', desc: '<strong>解題特徵引導：</strong>雙狹縫第 2 暗紋 $1.5\\Delta y = 1.95$ cm $\\Rightarrow \\Delta y = 1.30$ cm；單狹縫第 2 暗紋 $2\\frac{\\lambda L}{a} = 13.0$ cm $\\Rightarrow$ 中央亮寬為 $13.0$ cm。兩者相除 $\\frac{a}{d} = \\frac{1.30}{6.50} = \\frac{1}{5} \\Rightarrow a = 20.0\\ \\mu$m。' },
